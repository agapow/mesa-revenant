...
bool NexusToken::IsPunctuation( char ch )
{
   ...
   if(std::strchr( punctuation, ch ) != NULL )
      is_punctuation = true;
...
bool NexusToken::IsWhitespace( char ch )
{
	...
	//
	if(std::strchr( whitespace, ch ) != NULL )
		ws = true;
...
bool NexusToken::Abbreviation( nxsstring s )
{
	...
	for( k = 0; k < mlen; k++ )
	{
  		tokenChar = (char)std::toupper( token[k] );
  		otherChar = s[k];
	...
	{
  		tokenChar = (char)std::toupper( token[k] );
  		otherChar = (char)std::toupper( s[k] );
		if( tokenChar != otherChar )
			return false;
	}
 ...
 */
streampos_t  NexusToken::GetFilePosition()
{
	return filepos;
}
...
void NexusToken::ToUpper()
{
   for(unsigned int i = 0; i < token.size(); i++ )
      token[i] = (char)std::toupper( token[i] );
}
