	...
	, int seconds /* = 1 */ )
{
	assert( month != NULL );
	char mstr[10];
	int n = std::strlen(month);
	for( int i = 0; i < n; i++ )
		mstr[i] = toupper( month[i] );
	mstr[n] = '\0';
	
	ts.tm_mday = day;
	if( std::strcmp( month, "JANUARY" ) == 0 )
		ts.tm_mon = 0;
	else if(std::strcmp( month, "FEBRUARY" ) == 0 )
		ts.tm_mon = 1;
	else if(std::strcmp( month, "MARCH" ) == 0 )
		ts.tm_mon = 2;
	else if( std::strcmp( month, "APRIL" ) == 0 )
		ts.tm_mon = 3;
	else if( std::strcmp( month, "MAY" ) == 0 )
		ts.tm_mon = 4;
	else if( std::strcmp( month, "JUNE" ) == 0 )
		ts.tm_mon = 5;
	else if( std::strcmp( month, "JULY" ) == 0 )
		ts.tm_mon = 6;
	else if( std::strcmp( month, "AUGUST" ) == 0 )
		ts.tm_mon = 7;
	else if( std::strcmp( month, "SEPTEMBER" ) == 0 )
		ts.tm_mon = 8;
	else if( std::strcmp( month, "OCTOBER" ) == 0 )
		ts.tm_mon = 9;
	else if( std::strcmp( month, "NOVEMBER" ) == 0 )
		ts.tm_mon = 10;
	else if( std::strcmp( month, "DECEMBER" ) == 0 )
		ts.tm_mon = 11;
	else
		ts.tm_mon = -1;
