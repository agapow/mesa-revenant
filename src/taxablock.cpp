...
void TaxaBlock::Report( ostream& out )
{
	...
	for( int k = 0; k < ntax; k++ )
		out << '\t' << (k+1) << '\t' << taxonLabels[(unsigned long) k] << endl;
}
...
int TaxaBlock::GetMaxTaxonLabelLength()
{
   ...
   int maxlen = 0;
   for( int i = 0; i < ntax; i++ ) {
      int thislen = int (taxonLabels[(unsigned long) i].size());
      if( thislen > maxlen )
         maxlen = thislen;
   }
   return maxlen;
...
{
	assert( i < (int)taxonLabels.size() );
   return taxonLabels[(unsigned long) i];
}
