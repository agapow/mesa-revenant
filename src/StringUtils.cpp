...
double toDouble (const string& ikSrcStr)
...
long toLong (const string& ikSrcStr)
//: Attempt to convert & return the contents of the string as a double.
{
...
// The raison d'etre of these functions is to allow the safe construction 
// of file names, say by adding a suffix on to the end of the original
// file name. The optional argument sticks an ellipsis (...) where the 
// trimming took place.
...
bool StripExtension	(string& ioTargetString)
{
	...
	//DBG_MSG ("input to stripext is: " << ioTargetString);
	// step back along string until you find suffix start
	...
	if (ioTargetString[i] == '.')
	{
		ioTargetString.assign (ioTargetString.begin(), ioTargetString.begin() + i);
	//DBG_MSG ("input to stripext is: " << ioTargetString);
...
int iSizeLimit, bool iAddEllipsis)
{
	bool theResult = StripExtension (ioTargetString);
	ioTargetString += ".";
	ioTargetString += iSuffixCString;
	return theResult;
/*
...
bool hasExtension (string& ikTargetStr, const char* ikExtensionCStr)
{
	string theSearchTerm (".");
	theSearchTerm += ikExtensionCStr;
	return endsWith (ikTargetStr, theSearchTerm.c_str());
}


bool endsWith (string& ikTargetStr, const char* ikSuffixCStr)
{
	int theSuffixLength = int (std::strlen (ikSuffixCStr));
	assert (theSuffixLength <= ikTargetStr.size());
	
	string::iterator p = find_end (ikTargetStr.begin(), ikTargetStr.end(),
		ikSuffixCStr, ikSuffixCStr + theSuffixLength);
	if (p == (ikTargetStr.end() - theSuffixLength))
		return true;
	else
		return false;
}

bool beginsWith (string& ikTargetStr, const char* ikPrefixCStr)
{
	return (ikTargetStr.find (ikPrefixCStr) == 0);
}
