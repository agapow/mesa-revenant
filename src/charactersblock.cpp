...
char CharactersBlock::GetState( int i, int j, int k /* = 0 */ )
{
   ...
   int symbolsLen = int (std::strlen(symbols)); // PMA
...
{
   return bool (matrix->IsGap( i, j )); // PMA
}
...
{
   return bool (matrix->IsMissing( i, j)); // PMA
}
...
{
   return bool (matrix->IsPolymorphic( i, j)); // PMA
}
...
{
   assert( symbols != NULL );
   int symbolsLength = int (std::strlen(symbols)); // PMA
   int found = 0;
   for( int i = 0; i < symbolsLength; i++ )
   {
      char char_in_symbols = ( respectingCase ? symbols[i] : (char) std::toupper(symbols[i]) );
      char char_in_question = ( respectingCase ? ch : (char) std::toupper(ch) );
...
void CharactersBlock::HandleEliminate( NexusToken& token )
{
   ...
   nchar = ncharTotal - int (eliminated.size()); // PMA
...
void CharactersBlock::HandleFormat( NexusToken& token )
{
	...
	for(;;)
	{
		...
		else if( token.Equals("SYMBOLS") )
		{
			...
			for( llistsz_t i = 0; i < tlen; i++ ) //PMA
         {
				if( IsInSymbols( t[i] ) ) {
			...
			std::strcpy( symbols+numDefStates, t.c_str() );
...
bool CharactersBlock::HandleNextState( NexusToken& token, int i, int j )
{
   ...
   if( p != equates.end() ) {
	...
	else if( !tokens  && token.GetTokenLength() > 1 )
	{
		...
		nxsstring t = token.GetToken();
		nxsstring::size_type tlen = t.size(); // PMA
		int poly = ( t[0] == '(' );
		assert( poly || t[0] == '{' );
		assert( ( poly && t[tlen-1] == ')' ) || ( !poly && t[tlen-1] == '}' ) );
		
		nxsstring::size_type first_nonblank = 1; // PMA
		while( t[first_nonblank] == ' ' || t[first_nonblank] == '\t' )
			first_nonblank++;

		nxsstring::size_type last_nonblank = tlen-2; // PMA
...
void CharactersBlock::Report( ostream& out )
{
	...
	if( charLabels.size() > 0 )
	{
		...
		{
			if( charLabels[llistsz_t (k)].length() == 0 )
				out << '\t' << ( 1 + GetOrigCharIndex(k) ) << '\t' << "(no label provided for this character)" << endl;
			else
				out << '\t' << ( 1 + GetOrigCharIndex(k) ) << '\t' << charLabels[llistsz_t (k)] << endl;
         ...
         if( cib != charStates.end() )
         {
            ...
            for( int m = 0; m < ns; m++ ) {
         		out << "\t\t" << (*cib).second[llistsz_t (m)] << endl;
            }
         }
		}
	}
...
void CharactersBlock::ResetSymbols()
{
	...
	{
		case dna:
			std::strcpy( symbols, "ACGT" );
			break;
		case rna:
			std::strcpy( symbols, "ACGU" );
			break;
		case nucleotide:
			std::strcpy( symbols, "ACGT" );
			break;
		case protein:
			std::strcpy( symbols, "ACDEFGHIKLMNPQRSTVWY*" );
			break;
		default:
			std::strcpy( symbols, "01" );
	}
...
void CharactersBlock::WriteStates( DiscreteDatum& d, char* s, int slen )
{
   ...
   {
      assert( symbols != NULL );
      size_t symbolListLen = std::strlen( symbols );
