...
void TreesBlock::Read (NexusToken& token)
{
   isEmpty = false;
	...
	if (!token.Equals(";")) {
      ...
      errormsg += " instead";
		throw XNexus (errormsg, token.GetFilePosition(), token.GetFileLine(),
			token.GetFileColumn());
   }
	...
	for(;;)
	{
		...
		if (token.Equals("TRANSLATE"))
		{
         ...
         if (0 < numEntries)
         {
			...
			{
				// GOTCHA: will not cope with empty translation statements
				nxsstring theKey, theVal;
				while (not token.Equals(";"))
				{
					// get key
					// assert key is number?
		         token.GetNextToken();
					theKey = token.GetToken();			
	            if (token.Equals(";") or token.Equals(","))
	            {
						errormsg = "Number/name pair terminated early by \'";
						errormsg +=token.GetToken();
						errormsg += "\'";
                  throw XNexus (errormsg, token.GetFilePosition(), token.GetFileLine(), token.GetFileColumn());
	            }
					...
					// get label
	            token.GetNextToken();
	            nxsstring theVal = token.GetToken();
	            if (token.Equals(";") or token.Equals(","))
	            {
						errormsg = "Number/name pair terminated early by \'";
						errormsg +=token.GetToken();
	            ...
	            translateList[theKey] = theVal;
	            
	            // grab comma or semi-colon
					token.GetNextToken();
					if (token.Equals (";")) // end of statement
					{
						break;
					}
					else if (not token.Equals (",")) // if not a comma
					{
						errormsg = "Expecting ',' to terminate each number/name pair in TRANSLATE command, but found ";
						errormsg +=token.GetToken();
						errormsg += " instead";
         ...
         nxsstring sval = token.GetToken();
         // PMA: What? why is this spewing the tree to output?
         // cout << "\n***" << sval;

         // this should be a semicolon
         // PMA Ncl doesn't like lengths on the end of the tree (lengths from root)
         // although this is a perfectly legal (if useless) bit of information.
         token.GetNextToken();
         
         if (token.Equals(":")) // this block is mine
         {
				sval += ":";
				token.GetNextToken();
				sval += token.GetToken();
...
nxsstring TreesBlock::GetTreeName (int i)
{
	assert (i >= 0);
	assert (i < ntrees);
   return treeName[llistsz_t (i)]; // PMA
//trash	return treeList.GetKey(i);
}

static nxsstring& blanks_to_underscores (nxsstring& s)
{
   nxsstring::size_type len = s.length();
   for(nxsstring::size_type k = 0; k < len; k++) // PMA
   {
      if (s[k] == ' ')
         s[k] = '_';
   }
...
nxsstring TreesBlock::GetTranslatedTreeDescription (int i)
{
	...
	// the raw (untranslated) tree description
   nxsstring s = treeDescription[llistsz_t (i)];
   
   for (nxsstring::iterator q = s.begin(); q != s.end();)
   {
   	if (std::isspace (*q))
   		s.erase (q);
   	else
   ...
   nxsstring x;
   x += s[0];
   nxsstring::size_type slen = s.size();
   assert (slen > 1);
   
   for(nxsstring::size_type k = 1; k < slen; k++)
   {
      char prev = s[k-1];
      char curr = s[k];
      if (std::isdigit(curr) &&  (prev == '(' || prev == ','))
      {
         nxsstring ns;
         ns += curr;
         for(;;)
...
nxsstring TreesBlock::GetTreeDescription (int i)
{
	assert (i >= 0);
	assert (i < ntrees);
   return treeDescription[llistsz_t (i)];
...
int TreesBlock::IsRootedTree (int i)
{
	assert (i >= 0);
	assert (i < ntrees);
	return (int)rooted[llistsz_t (i)]; // PMA
}
