...
nxsstring AllelesBlock::GetLocusLabel( int locus )
{
	assert( locus >= 0 );
	assert( locus < nchar );
	return charLabels[llistsz_t (locus)];
}
...
nxsstring AllelesBlock::GetAlleleLabel( int locus, int allele )
{
   ...
   {
      int ns = int ((*cib).second.size()); // PMA
      if( allele >= 0 && allele < ns )
      	allele_label = (*cib).second[llistsz_t (allele)]; // PMA
   }
   return allele_label;
}
...
int AllelesBlock::HandleAllele( NexusToken& token, int j )
{
   ...
   int nAlleles = int ((*bagIter).second.size()); // PMA
...
void AllelesBlock::Report( ostream& out )
{
   ...
   out.flush();
	...
	int numEquateMacros = int (equates.size()); // PMA
	...
	if( charLabels.size() > 0 )
	{
		...
		for(llistsz_t k = 0; k < nchar; k++ ) { // PMA
			if( charLabels[k].size() == 0 )
				...
				out << '\t' << ( 1 + GetOrigCharIndex(int (k)) ) << '\t' << charLabels[llistsz_t (k)] << endl;
...
int AllelesBlock::SplitInt( int x, int y )
{
   ...
   int z_high = (int) ( (unsigned int)( (unsigned short)y ) ) << 16;
   int z = ( z_low | z_high );
   return z;
}
