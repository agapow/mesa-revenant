 ...
 * @variable   countCols() [int:public] number of rows (taxa) in the data matrix
 ...
 * <tr><td> countRows()   <td>= <td> rows
 * <tr><td> countCols()    <td>= <td> cols
 * </table>
 ...
 */
DiscreteMatrix::DiscreteMatrix (int iRows, int iCols)
{
	datavec_t	theDataRow ((datavec_t::size_type) iCols);	
	for (datamatrix_t::size_type i = 0; i < (unsigned int) iRows; i++)
		data.push_back (theDataRow);
}
...
int DiscreteMatrix::countRows()
{
	return int (data.size());
}

int DiscreteMatrix::countCols()
{
	if (countRows() == 0)
		return 0;
	else
 ...
 * countRows().  Data already stored in data is not destroyed; the newly-allocated
 * rows are added at the bottom of the existing matrix.
 */
void DiscreteMatrix::AddRows(int iNewRows)
{
	datavec_t	theDataRow ((datavec_t::size_type) countCols());	
	for (datamatrix_t::size_type i = 0; i < (unsigned int) iNewRows; i++)
		data.push_back (theDataRow);

}

void DiscreteMatrix::addCols (int iColIncr)
// add new columns
{
	int theNumCols = countCols() + iColIncr;
	for (datamatrix_t::size_type i = 0; i < data.size(); i++)
		data[i].resize (theNumCols);
}
 ...
 * Assumes i is in the range [0..countRows()) and j is in the range [0..countCols()).
...
{
   out << endl;
   out << "countRows() = " << countRows() << endl;
   out << "countCols() = " << countCols() << endl;
 ...
 * to 0 and countCols(), so if duplication of the entire row is needed, these need not
   ...
   , int startCol /* = 0 */, int endCol /* = -1 */)
{
	...
	assert(startCol >= 0);
	assert(startCol < countCols());
   if(endCol == -1)
      endCol = countCols()-1;
	assert(endCol > startCol);
	assert(endCol < countCols());

   // expand matrix (if necessary) to accommodate additional rows
   //
   int nNewRows = 0;
   if(row + count > countRows()) {
      nNewRows = row + count - countRows();
      AddRows(nNewRows);
   }
 ...
 *
 * Deletes all cells of data and resets countRows() and countCols() to 0.
 */
 ...
 *
 * Assumes that i is in the range [0..countRows()) and j is in the range [0..countCols()).
 ...
 * Assumes j is in the range [0..countCols()).
 */
 ...
 * gap symbol, 0 otherwise.  Assumes i is in the range [0..countRows()) and j is
 * in the range [0..countCols()).
 */
 ...
 *
 * Returns 1 if the state for taxon i, character j, is set to the
 * missing data symbol, 0 otherwise.  Assumes i is in the range [0..countRows())
 * and j is in the range [0..countCols()).
 */
 ...
 * i is in the range [0..countRows()) and j is in the range [0..countCols()).
 */
 ...
 * a new matrix object with countRows() = rows and countCols() = cols.
 */
void DiscreteMatrix::Reset(int iRows, int iCols)
{
   assert(iRows > 0);
   assert(iCols > 0);

	data.clear();
	datavec_t	theDataRow (iCols);	
	for (datamatrix_t::size_type i = 0; i < (unsigned int) iRows; i++)
		data.push_back (theDataRow);
}
 ...
 * Assumes i is in the range [0..countRows()) and j is in the range
 * [0..countCols()).
 */
 ...
 * will set polymorphic to 1 in data[i][j]. Assumes i is in the range [0..countRows())
 * and j is in the range [0..countCols()).  Also assumes that the number of states
